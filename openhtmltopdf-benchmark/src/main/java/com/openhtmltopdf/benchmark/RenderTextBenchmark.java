...
public class RenderTextBenchmark {
    ...
    public void setUp() {
        ...
        Arrays.asList(
                "/html/render-text-plain.html",
                "/html/render-text-soft-hyphens.html"
        ).forEach(path -> contents.put(path, readContent(path)));
    }

    @Benchmark
    public void renderText_Plain() throws Exception {
        runRenderer(contents.get("/html/render-text-plain.html"));
    }

    @Benchmark
    public void renderText_SoftHyphens() throws Exception {
        runRenderer(contents.get("/html/render-text-soft-hyphens.html"));
    }
